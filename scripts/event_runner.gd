...
func start_chapter(path: String) -> void:
	if not ChapterManager.load_chapter(path):
...
func continue_chapter() -> void:
	var guard := 0
	while guard < 1000:
		var event := ChapterManager.get_current_event()
		if event.is_empty():
			push_error("Missing event: %s" % ChapterManager.current_event_id)
			await UIManager.show_main_menu()
			return
		...
		ChapterManager.set_current_event(next_event_id)
		SaveManager.save_game()
		guard += 1
...
func process_event(event: Dictionary) -> String:
	...
	match kind:
		"story", "narration", "choice":
			return await UIManager.show_story_event(event)
		"mechanic":
			return await _run_mechanic_event(event)
		"reward":
			StatePathUtil.apply_changes(event.get("changes", []))
			SaveManager.save_game()
		...
		"state_change":
			...
			return event.get("next", "")
		"conditional":
			var passed := StatePathUtil.evaluate_condition(event.get("condition", {}))
			return event.get("true_next" if passed else "false_next", "")
		"jump":
			return event.get("next", "")
		"end":
			SaveManager.save_game()
			await UIManager.show_chapter_complete({
...
func _run_mechanic_event(event: Dictionary) -> String:
	...
	var result := await MechanicRegistry.run_mechanic(mechanic_id, event.get("params", {}))
